...
class QueryEngine(Credentials, SPARQL, HelperContract):
    ...
    def get_software_by_id(self, id):
        query = textwrap.dedent("""{0}
            SELECT ?softwareId ?name ?description ?createDate ?licenseId ?versionInfo   
                WHERE {{ 
                ?Software rdf:type dc:Software;
                :softwareID ?softwareId;
                :hasName ?name;
                dct:description ?description;
                :hasCreationDate ?createDate;
                :licenseID ?licenseId;
                dc:hasVersion ?versionInfo .
                filter(?softwareId="{1}") .
            }}""").format(self.prefix(), id)
    ...
    def get_all_software(self):
        query = textwrap.dedent("""{0}
            select ?softwareId ?name ?description ?licenseId ?versionInfo ?createDate
            where{{  
                ?Software rdf:type dc:Software;
                :softwareID ?softwareId;
                :licenseID ?licenseId;
    ...
    def insert_query(self, ContractId, ContractType, Purpose,
                     ...
                     ConsiderationDescription, ConsiderationValue, Contractors, Terms, Signatures, LicenseId):
    ...
    def insert_query_software(self, SoftwareId, Name, Description, CreateDate, LicenseId, VersionInfo):
        # create_date = datetime.now()
        insquery = textwrap.dedent("""{0} 
           INSERT DATA {{
               :{1} rdf:type dc:Software;
                           :hasName "{2}";
                           dct:description "{3}";
                           :hasCreationDate {4};
                           :licenseID "{5}";
             ...
             """.format(self.prefix(), SoftwareId, Name, Description, '\'{}\'^^xsd:dateTime'.format(CreateDate), LicenseId, VersionInfo))
        return insquery
